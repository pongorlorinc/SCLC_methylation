...
BinCoordinates = function(bw, coord, nbins, remove_NA = T) {
  ...
  coord.tiles$score = ScoreGrangesBWmeanMean(coords = coord.tiles, bw = bw)
...
# this file can be downloaded from SCLC-CellMinerCDB
...
histone_bigwig = "../data/NCI-H69_H3K27ac_rep1.hg19_clean.bam.scaled.bw"

# CpG island file (used to select transcript)
cpg_file = "../data/hg19.cpg.bed"
...
rnadf$quantile = ifelse(rnadf$mean > 1, 1, 0) # mark non expressed genes (FPKM <= 1) as 0 quantile
...
# import gene signal only (these files can be big depending of size)
histbw = rtracklayer::import(histone_bigwig, which = annotation.gr + 2 * (upstream + downstream))
...
for(i in 1:length(quants)) {
  ...
  df.dt = data.table(df)
  df.dt = df.dt[,list(score = mean(score)), by=bin]
  df = as.data.frame(df.dt)
  df$quantile = quants[i]
  
  if(nrow(meth_signal_df) == 0) {
    meth_signal_df = df
  } else {
    meth_signal_df = rbind(meth_signal_df, df)
  }
}

histone_signal_df = data.frame()
for(i in 1:length(quants)) {
  print(i)
  ...
  gc()
  # create bin matrix for upstream area
  dfs[[1]] = BinCoordinates(bw = histbw, coord = geneupstream[geneupstream$gene %in% genes], nbins = up_and_downstraem_bins)
  
  # create bin matrix for promoter area
  dfs[[2]] = BinCoordinates(bw = histbw, geneproms[geneproms$gene %in% genes], promoterbins)
  
  # create bin matrix for gene body area
  dfs[[3]] = BinCoordinates(histbw, genebody[genebody$gene %in% genes], bodybins)
  
  # create bin matrix for downstream area
  dfs[[4]] = BinCoordinates(histbw, genedownstream[genedownstream$gene %in% genes], up_and_downstraem_bins)
  
  for(j in 1:4) {
    df.dt = data.table(dfs[[j]])
    df.dt = df.dt[,list(score = mean(score)), by=bin]
    dfs[[j]] = as.data.frame(df.dt)
  }
  ...
  if(nrow(histone_signal_df) == 0) {
    histone_signal_df = df
  } else {
    histone_signal_df = rbind(histone_signal_df, df)
  }
  gc()
}

meth_signal_df$quantile = as.character(meth_signal_df$quantile)
ggplot(meth_signal_df, aes(x = factor(bin), y = score, group = quantile, color = quantile)) +
  geom_line() +
  ylim(0,1) +
...
histone_signal_df$quantile = as.character(histone_signal_df$quantile)
ggplot(histone_signal_df, aes(x = factor(bin), y = score, group = quantile, color = quantile)) +
  geom_line() +
  theme(axis.text.x=element_blank(), axis.title.x=element_blank()) +
  theme_classic() +
  theme(axis.title.x=element_blank(), axis.text.x=element_blank(),axis.ticks.x=element_blank())
